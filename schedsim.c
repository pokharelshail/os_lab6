...
/*Shreeshail Pokharel @02931612 Collaborated with Nikesh, Aayush, Manish*/
#include<stdio.h> 
#include<limits.h>
#include<stdlib.h>
#include "process.h"
...
{ 
  ...
  int left_time[n];
  for (int i=0; i<n; i++) {
    left_time[i] = plist[i].bt;
  }
  int curr_time = 0;
  int finished = 0;
  while (finished != n) {
    for (int i=0; i<n; i++) {
      if (left_time[i] > quantum) {
        curr_time += quantum;
        left_time[i] -= quantum;
      } else if (left_time[0] > 0) {
        finished++;
        curr_time += left_time[i];
...
{ 
  ...
  while (finished != n) {
    ...
    for (int i=curr_proc+1; i<n; i++) {
      if (curr_time >= plist[i].art) {
        if (left_time[i] != 0 && left_time[i] < left_time[curr_proc]) {
          curr_proc = i;
        }
      }
    }
    if (curr_time >= plist[0].art) {
      left_time[curr_proc]--;
      curr_time++;
      if (left_time[curr_proc] == 0) {
        finished++;
        plist[curr_proc].tat = curr_time;
        plist[curr_proc].wt = curr_time - plist[curr_proc].bt - plist[curr_proc].art;
      }
    } else {
      curr_time++;
    }
